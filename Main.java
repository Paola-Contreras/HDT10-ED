        ...
        while (salir==false){
            ...
            if(menusito==1){
                ...
                while(salir2==false){
                    ...
                    if(menusito3==1){
                        ...
                        if (grafo.existe(Csalida,Cdestino)){
                            v.rute();
                            grafo.cerca(Csalida, Cdestino); // Check the cities that are coloser 
                ...
                while(salir1==false){
                    ...
                    // option of traffic disruption
                    if(menusito2==1){
                        v.Departure(); // Ask for the first city
                        Csalida=scan.nextLine();

                        v.Arrive();// Ask for the first city
                        Cdestino=scan.nextLine();
                        // If that checks if both cities exist 
                        if (grafo.existe(Csalida, Cdestino)){
                            grafo.agregar(Csalida,Cdestino,"100"); // add tho the graph the new data 
                        ...
                        v.Departure();
                        Csalida=scan.nextLine(); // Ask for the first city

                        v.Arrive(); // Ask for the second city
                        Cdestino=scan.nextLine();

                        v.distance(); // Ask for the distance
                        caminokm=scan.nextLine();
                        // If that checks if both cities exist
                        if (grafo.existe(Csalida, Cdestino)){
                            grafo.agregar(Csalida,Cdestino,caminokm); // add tho the graph the new data
                            //Shows the data changed 
                            v.exito();
                            System.out.println("Ciudades Modificadas: " +"\n-"+ Csalida + "\n-"+ Cdestino );
